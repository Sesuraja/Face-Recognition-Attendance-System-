...
class ChatBot:
	def __init__(self,root):
		...
		self.root.geometry("730x950+0+0")
		self.root.bind('<Return>',self.enter_func)

		main_frame=Frame(self.root,bd=3,bg='powder blue',width=610)
		main_frame.pack()

		img_chat=Image.open(r"F:\machine\img\chatIcon.jpg")
		img_chat=img_chat.resize((200,70),Image.ANTIALIAS)
		self.photoimg=ImageTk.PhotoImage(img_chat)

		Title_label=Label(main_frame,bd=3,relief=RAISED, anchor='nw',width=730, compound=LEFT, image=self.photoimg,text='CHAT ME',font=('times new roman',30,'bold'),fg='red',bg='white')
		...
		self.send=Button(btn_frame,text="Send>>",command=self.send,font=('times new roman',16,'bold'),width=8,bg='white')
	...
	def send(self):
		...
		if (self.entry.get()=='hello'):
			...
			self.text.insert(END,"\n\n"+"Bot: sesuraja, a student of Computer Science from M.A.M")
